...
#ifndef USE_ROCM
  return __halves2bfloat162(__float2bfloat16_rn(silu(x.x)),
...
void silu_and_mul_quant(torch::Tensor& out,    // [..., d]
  ...
  if (input.dtype() == torch::kBFloat16) {
    ...
    TORCH_CHECK(props->major >= 8,
                "silu_and_mul_quant with bfloat16 requires CUDA compute "
                "capability >= 8.0. Use float16 on this GPU.");
  }
#endif
...
void persistent_masked_m_silu_mul_quant(
  ...
  const auto* props = at::cuda::getCurrentDeviceProperties();
  TORCH_CHECK(props->major >= 8,
              "persistent_masked_m_silu_mul_quant requires CUDA compute "
              "capability >= 8.0 because it uses bfloat16 device intrinsics.");
